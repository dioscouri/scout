...
class plgScoutComModules extends JObject 
{
	var $_loadItem = false;
	
    function plgScoutComModules()
    {
        parent::__construct();
    }
    ...
    function createLogEntry()
    {
        ...
        if (!$log->save())
        {
        	JError::raiseNotice( 'plgScoutComModules01', "plgScoutComModules :: ". $log->getError() );
        }
    ...
    function getVerb()
    {
        //        tasks from the J1.5 com_modules controller
        //          save, publish, remove PLUS:
    ...
    function getObject()
    {
        ...
        {
            case "1": // admin
                $scope_url = 'index.php?option=com_modules&task=edit&cid[]=';
                break;
            case "0": // site
            default:
        ...
        if (empty($array[0]))
        {
	        ...
	        // is it logical to query the #__modules DB for the next autoincrement id?  what if the save fails?
	        $db = JFactory::getDBO();
	        $tbl = $db->replacePrefix( '#__modules' );
        ...
        $return =
            array(
                ...
                'scope_identifier'=>'com_modules&view=module',  // required. is unique to this site+component+view(+layout) combo
                'scope_name'=>'Module Manager',        // optional. only necessary if this scope is a new one
