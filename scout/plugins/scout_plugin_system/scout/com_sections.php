...
class plgScoutComSections extends JObject 
{
	var $_loadItem = false;
	
    function plgScoutComSections()
    {
        parent::__construct();
    }
    ...
    function createLogEntry()
    {
        ...
        if (!$log->save())
        {
        	JError::raiseNotice( 'plgScoutComSections01', "plgScoutComSections :: ". $log->getError() );
        }
    ...
    function getObject()
    {
        ...
        {
            case "1": // admin
                $scope_url = 'index.php?option=com_sections&task=edit&cid[]=';
                break;
            case "0": // site
            default:
                return false; // don't do anything for the site w/ com_sections
                break;
        }
        ...
        {
	        $db = JFactory::getDBO();
	        $tbl = $db->replacePrefix( '#__sections' );
        ...
        $return =
            array(
                ...
                'scope_identifier'=>'com_sections&view=item',  // required. is unique to this site+component+view(+layout) combo
                'scope_name'=>'Section Manager',        // optional. only necessary if this scope is a new one
