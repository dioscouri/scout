...
class plgScoutComPlugins extends JObject 
{
	var $_loadItem = false;
	
    function plgScoutComPlugins()
    {
        parent::__construct();
    }
    ...
    function createLogEntry()
    {
        ...
        if (!$log->save())
        {
        	JError::raiseNotice( 'plgScoutComPlugins01', "plgScoutComPlugins :: ". $log->getError() );
        }
    ...
    function getObject()
    {
        ...
        {
            case "1": // admin
                $scope_url = 'index.php?option=com_plugins&task=edit&cid[]=';
                break;
            case "0": // site
            default:
                return false; // don't do anything for the site w/ com_plugins
                break;
        }
        ...
        {
	        $db = JFactory::getDBO();
	        $tbl = $db->replacePrefix( '#__plugins' );
        ...
        $return =
            array(
                ...
                'scope_identifier'=>'com_plugins&view=plugin',  // required. is unique to this site+component+view(+layout) combo
                'scope_name'=>'Plugin Manager',        // optional. only necessary if this scope is a new one
