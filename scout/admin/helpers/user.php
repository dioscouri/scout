...
class ScoutHelperUser extends DSCHelperUser
{
     ...
     * 
     * @param int $userid
     * @return obj ScoutAddresses if found, false otherwise
     */
	...
	function getPrimaryAddress( $userid, $type='billing' )
	{
		...
		$model = JModel::getInstance( 'Addresses', 'ScoutModel' );
		switch($type)
		{
			case "shipping":
	...
	function &createNewUser( $details, &$msg ) {
		...
		ScoutHelperUser::_sendMail( $user, $details, $useractivation );
	...
	function _sendMail( &$user, $details, $useractivation ) {
		...
		$success = ScoutHelperUser::_doMail($mailfrom, $fromname, $email, $subject, $message);
