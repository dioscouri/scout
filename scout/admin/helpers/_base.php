...
class ScoutHelperBase extends JObject
{   
	/**
	...
	function currency($amount, $currency='', $options='')
	{
        // default to whatever is in config
            $config = ScoutConfig::getInstance();
		...
		if (empty($items[$start_datetime][$end_datetime][$period][$select]))
		{
			...
			while ($curdate <= $enddate)
			{
				// set working variables
					$variables = ScoutHelperBase::setDateVariables( $curdate, $enddate, $period );
	...
	{
        self::includeJQuery();
	    JHTML::_('script', 'jquery-ui-1.7.2.min.js', 'media/com_scout/js/');
        JHTML::_('stylesheet', 'jquery-ui.css', 'media/com_scout/css/');
	}
	...
	{
	    JHTML::_('script', 'jquery-1.3.2.min.js', 'media/com_scout/js/');
	}
	 ...
	 * @access public
	 * @param mixed $data
	 * @param string $title. (default: 'A Scout Google Chart')
	 * @param string $type. (default: 'Column')
	 * @param int $width. (default: 900)
	 * @param int $height. (default: 250)
	 * @return void
	 */
	function renderGoogleChart($data, $title='A Scout Google Chart', $type='Column', $width=800, $height=250)
    {
        $title  = JText::_( $title );
