...
// TODO Make all Scout plugins extend this _base file, to reduce code redundancy

/** Import library dependencies */
jimport('joomla.event.plugin');
jimport('joomla.utilities.string');

class ScoutPluginBase extends JPlugin
{
    /**
     ...
     * @access protected
     */
    function _getLayout($layout, $vars = false, $plugin = '', $group = 'scout' )
    {
        if (empty($plugin)) 
        {
    ...
    function _displayArticle()
    {
        ...
        if ($articleid)
        {
            JLoader::import( 'com_scout.library.article', JPATH_ADMINISTRATOR.DS.'components' );
            $html = ScoutArticle::display( $articleid );
        }
