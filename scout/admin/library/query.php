 ...
 * @since		1.6
 */
class ScoutQueryElement extends JObject
{
 ...
 * @since		1.6
 */
class ScoutQuery extends JObject
{
	...
	public function select($columns)
	{
		...
		if (is_null($this->_select)) {
			$this->_select = new ScoutQueryElement('SELECT', $columns);
		} else {
    ...
    {
        $this->_type = 'delete';
        $this->_delete = new ScoutQueryElement('DELETE', array(), '');
        return $this;
    }
    ...
    {
        $this->_type = 'insert';
        $this->_insert = new ScoutQueryElement('INSERT INTO', $tables);
        return $this;
    }
    ...
    {
        $this->_type = 'update';
        $this->_update = new ScoutQueryElement('UPDATE', $tables);
        return $this;
    }
	...
	public function from($tables)
	{
		if (is_null($this->_from)) {
			$this->_from = new ScoutQueryElement('FROM', $tables);
		} else {
	...
	public function join($type, $conditions)
	{
		...
		$this->_join[] = new ScoutQueryElement(strtoupper($type) . ' JOIN', $conditions);
    ...
    public function set($conditions, $glue=',')
    {
        if (is_null($this->_set)) {
            ...
            $this->_set = new ScoutQueryElement('SET', $conditions, "\n\t$glue ");
        } else {
	...
	public function where($conditions, $glue='AND')
	{
		if (is_null($this->_where)) {
			$glue = strtoupper($glue);
			$this->_where = new ScoutQueryElement('WHERE', $conditions, "\n\t$glue ");
		} else {
	...
	public function group($columns)
	{
		if (is_null($this->_group)) {
			$this->_group = new ScoutQueryElement('GROUP BY', $columns);
		} else {
	...
	public function having($conditions, $glue='AND')
	{
		if (is_null($this->_having)) {
			$glue = strtoupper($glue);
			$this->_having = new ScoutQueryElement('HAVING', $conditions, "\n\t$glue ");
		} else {
	...
	public function order($columns)
	{
		if (is_null($this->_order)) {
			$this->_order = new ScoutQueryElement('ORDER BY', $columns);
		} else {
